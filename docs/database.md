...
History is preserved: changing plans ends the current subscription and inserts a new row rather than mutating in place, so the analytics endpoint can report plan history and churn. The ended row becomes `canceled` with `canceled_at` and `ends_at` set to the moment of the change. Cancelling without a change sets `canceled_at` and `ends_at` (the first billing-period boundary after now, counted from `starts_at` in PHP): the row stays `active` until `ends_at`, so it still holds the one-live-subscription slot.
...
- **Changing a tenant's timezone** would bucket later customers by the new zone and leave earlier months as stored. No endpoint changes it today.
...
`0` is a meaningful limit (feature disabled). A sentinel like `-1` would sort and aggregate incorrectly. Null is the honest representation of "no ceiling", and the limit check tests for it with `is_null()` before counting.
...
Migrations are ordered so foreign keys always resolve: `tenants` → `users` → `plans` → `plan_features` → `subscriptions` → `customers` → `tenant_stats` → `platform_stats` → `plan_stats` → `tenant_monthly_stats`.
...
| `GET /subscription`, `/tenant`, `/auth/me` on a warm cache | 0 queries | 0 queries | Redis |
