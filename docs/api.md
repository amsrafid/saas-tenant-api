...
**Risk, stated plainly.** Scramble is pre-1.0 (v0.13.x as of Aug 2026). Its Laravel 13 support is verified at install time in `S1-02`. If it does not resolve against Laravel 13, the fallback is a **hand-written `openapi.yaml`** served through the same Swagger UI — roughly an extra hour, still Swagger, still no annotations in the controllers, but it must be kept in step with the code by hand. L5-Swagger remains rejected either way, for the reasons above. The decision is reversible at that point and nowhere later.
